...
# Get the column names of the income_statement table (if i want to change the years, update the [1:3] part)
cursor.execute("SELECT * FROM income_statement LIMIT 1")
columns = [description[0] for description in cursor.description[1:3]]
...
for column in columns:
    ...
    cursor.execute(f"SELECT [{column}] FROM income_statement WHERE Account = 'Gross profit'")
    gross_profit = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM income_statement WHERE Account = 'Sales, net'")
    sales_net = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM balance_sheet WHERE Account = 'Total current assets'")
    total_current_assets = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM balance_sheet WHERE Account = 'Inventories'")
    inventories = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM balance_sheet WHERE Account = 'Total current liabilities'")
    total_current_liabilities = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM balance_sheet WHERE Account = 'Total liabilities'")
    total_liabilities = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM balance_sheet WHERE Account = 'Total stockholders'' equity'")
    total_stockholders_equity = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM income_statement WHERE Account = 'Net income'")
    net_income = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM balance_sheet WHERE Account = 'Long-term debt, net'")
    longterm_debt = float(cursor.fetchone()[0])

    cursor.execute(f"SELECT [{column}] FROM balance_sheet WHERE Account = 'Current portion of long-term debt'")
